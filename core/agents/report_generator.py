...
def generate_report(state: ConstructionState) -> dict:
    """
    ...
    try:
        ...
        report_data = {
            "executive_summary": {
                ...
                "total_cost": f"${state.total_cost + cost:.4f}"
            },
            "violations": [
                {
                    "id": v.violation_id,
            ...
            "cost_breakdown": {
                ...
                "total_cost_usd": round(state.total_cost + cost, 4),
                "cost_per_violation": round((state.total_cost + cost) / max(len(state.violations), 1), 4),
                "under_budget": (state.total_cost + cost) <= BUSINESS_CONFIG["cost_per_site_budget"]
            },
            "permit_info": {
        ...
        errors.append(f"report_generator failed: {str(e)}")
